---
title: "multiparcel distance analysis"
...
* Subjects: `r length(subjs.complete)` included (with complete fMRI data)
* INCLUDES TWINS
...
loocv_lda_proj <- function(data_matrix, .whiten) {
  ...
  for (subj.i in seq_len(dim(data_matrix)[1])) {
    ...
    rotation_matrix <- lda_proj(X_train, .whiten)
    
    X_train_proj <- crossprod(X_test, rotation_matrix)
...
data_matrix_c <- aperm(data_matrix_c, c(1, 3, 2))
dim(data_matrix_c) <- c(prod(dim(data_matrix_c)[1:2]), dim(data_matrix_c)[3])  ## concatenate sessions along rows
Winv <- pracma::sqrtm(invcov.shrink(data_matrix_c, verbose = FALSE))$B
...
for (subj.i in seq_len(dim(X)[1])) {
  ...
  D[, , subj.i] <- distance_cv(t(Winv %*% x_train), t(Winv %*% x_test), M, regressors = sessions)
